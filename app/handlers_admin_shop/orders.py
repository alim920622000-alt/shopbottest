...
async def build_order_card_payload(
    ...
    business_type = (shop_info or {}).get("business_type") or "shop"
    emoji = business_emoji(business_type)
    ...
    lines = [
        ...
        "",
        admin_action_line("shop", o.get("merchant_status"), o.get("courier_status")),
        "",
...
async def order_card(cq: CallbackQuery, db: Database, state: FSMContext):
    ...
    if is_chat_reminder_text(cq.message.text if cq.message else None):
        ...
        await safe_edit_text(cq.message, text, reply_markup=kb, parse_mode="HTML")
    if found:
