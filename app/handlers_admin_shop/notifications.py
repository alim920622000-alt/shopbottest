...
async def _render_center(cq: CallbackQuery, db: Database, state: FSMContext) -> None:
    ...
    await show_screen(cq.bot, cq.from_user.id, state, db, "admin_shop", text, kb, screen_kind="notif_center")
...
async def notif_return(cq: CallbackQuery, db: Database, state: FSMContext) -> None:
    lock = get_admin_screen_lock("admin_shop", cq.from_user.id)
    async with lock:
        ...
        if not target or target == "a:home":
            ...
            await ensure_admin_screen_state(db, "admin_shop", cq.from_user.id, cq.message.message_id if cq.message else None)
            await cq.answer()
            return
        if target == "a:orders":
